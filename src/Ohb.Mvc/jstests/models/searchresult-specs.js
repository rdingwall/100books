...
$(function () {
    ...
    (function (
        ...
        describe("SearchResult", function () {
            ...
            describe("Getting the SearchResult's view URL", function () {
                it("should return a stub of the title with the prefix", function () {
                    var searchResult = new SearchResult({ id: 42, title: "Harry Potter" });
                    expect(searchResult.get("viewUrl")).toEqual("#books/42/harry-potter");
                });
            });

            describe("Fetching search results from Google Books API", function () {

                var assertBookMapping = function (callback, googleBookId, title, authors, smallThumbnailUrl, stub) {
                ...
                it("should include the book's subtitle", function () {
                    runsAsync(function (callback) {
                        assertBookMapping(callback, "abYKXvCwEToC",
                            "Harry Potter: The Story of a Global Business Phenomenon",
                            "Susan Gunelius",
                            "http://bks6.books.google.co.uk/books?id=abYKXvCwEToC&printsec=frontcover&img=1&zoom=5&edge=curl");
                    });
                });

                it("should list multiple authors", function () {
                    runsAsync(function (callback) {
                        assertBookMapping(callback, "GGpXN9SMELMC", "Head First Design Patterns",
                            "Eric Freeman, Elisabeth Freeman, Kathy Sierra, Bert Bates",
                            "http://bks9.books.google.co.uk/books?id=GGpXN9SMELMC&printsec=frontcover&img=1&zoom=5&edge=curl");
                    });
                });

                it("should handle books without thumbnails", function () {
                    runsAsync(function (callback) {
                        assertBookMapping(callback, "DAAAAAAACAAJ",
                            "Harry Potter: 5 Years of Magic, Adventure, and Mystery at Hogwarts", "J. K. Rowling",
                ...
                it("should handle books with no authors", function () {
                    runsAsync(function (callback) {
                        assertBookMapping(callback, "3zgFSQAACAAJ", "The Girl With the Dragon Tattoo", null,
                            "http://bks1.books.google.co.uk/books?id=3zgFSQAACAAJ&printsec=frontcover&img=1&zoom=5");
                    });
                });
